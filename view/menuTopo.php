<button class="navBtn"><a href="<?='http://localhost/projeto-php-ete-1/?pagina=login'?>">Login</a></button>
<button class="navBtn"><a href="<?='http://localhost/projeto-php-ete-1/?pagina=registro'?>">Registro</a></button>
<button class="navBtn"><a href="<?=constant('URL_LOCAL_SITE_PAGINA').'contato'?>">Contato</a></button>
<button class="navBtn"><a href="<?=constant('URL_LOCAL_SITE_PAGINA').'cadastrar-noticia'?>">Cadastrar noticias</a></button>
<button class="navBtn"><a href="<?=constant('URL_LOCAL_SITE_PAGINA').'cadastrar-categoria'?>">Cadastrar categorias</a></button>
<button class="navBtn"><a href="<?=constant('URL_LOCAL_SITE_PAGINA').'doacao'?>">Doação</a></button>
<button class="navBtn"><a href="<?=constant('URL_LOCAL_SITE_PAGINA').'sair'?>">Sair</a></button>